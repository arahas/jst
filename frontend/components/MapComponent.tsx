...
export default function MapComponent({ data }: MapComponentProps) {
  ...
  // Create a unique key for the MapContainer that changes when data changes
  const mapKey = useRef(Date.now()).current
  const [mapInstanceKey, setMapInstanceKey] = useState(mapKey)
  
  // Set default center and zoom
  const defaultCenter: [number, number] = [39.8283, -98.5795] // Center of US
  const defaultZoom = 4
  
  // Prepare map when component mounts
  useEffect(() => {
  ...
  useEffect(() => {
    // Generate a new key to force MapContainer to remount
  ...
  return (
    <div className="h-[600px] w-full rounded-lg overflow-hidden border border-gray-300">
      {mapReady && (
        <MapContainer
          key={mapInstanceKey} // Add a key that changes when data changes
